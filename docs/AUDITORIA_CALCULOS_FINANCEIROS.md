...
Justificativa: O sistema tem código funcional em cenários simples, mas apresenta falhas objetivas em parser crítico, regras de adicionais potencialmente ilegais, duplicação de dias no VR e ausência de testes automatizados. A governança do banco (tabelas sem migration) e a fragilidade dos comprovantes de pagamento elevam o risco operacional e trabalhista. Recomenda-se **não usar o módulo de adicionais para fechamento até a correção do parser e da regra dos 30 dias**, e revisar manualmente toda geração de VR até os testes automatizados estarem em vigor.
