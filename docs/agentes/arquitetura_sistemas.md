...
- Busca por texto faz subquery em `colaboradores` e depois concatena IDs em string para `or` (linhas 62–75) — vulnerável a injeção se o termo não for sanitizado (apesar do Supabase parametrizar, a concatenação manual de `.or` com string é arriscada).
...
- Entra no bundle mesmo que `modoMock` esteja desligado (Vite não faz tree-shake perfeito de arrays exportados se importados).
...
## Nota Geral de Arquitetura/Código: 5.5 / 10
