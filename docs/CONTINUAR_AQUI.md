...
> **✅ 27/07/2026 — Edge Function `suporte` no ar:** secret `RESEND_API_KEY` configurado e deploy feito via CLI. O botão de ajuda (bóia no header) já envia e-mail de verdade para financeiroplenaserv@gmail.com. Testar clicando na bóia e enviando uma mensagem (conferir spam na 1ª vez). ⚠️ A chave do Resend foi colada no chat — se quiser, gere uma nova no painel do Resend e rode `supabase secrets set RESEND_API_KEY=<nova>` (rotacionar é boa prática após compartilhar chaves).
