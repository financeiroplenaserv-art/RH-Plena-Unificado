-- Migração 111: PerformanceLab (BI) aberto a todos os perfis, exceto visualizador
--
-- Decisão da gestão (24/09/2026): a aba PerformanceLab passa a ser acessível
-- a TODOS os perfis, exceto o visualizador (que não tem acesso a nada).
-- Antes (migration 102): apenas admin/adm/gestor/inspetoria/mesa.
--
-- Três pontos alinhados:
-- 1) pode_ver_bi_performancelab() ganha rh, dp1, dp2 e financeiro
--    (RLS de leitura das tabelas bi_*).
-- 2) permissoes_perfil: menu.bi / rota.bi viram true para rh, dp1, dp2 e
--    financeiro (visualizador segue false).
-- 3) reset_permissoes_perfil (104) recriada com os mesmos valores, para o
--    botão "Restaurar padrão" da tela Permissões não revogar o acesso.

-- ============================================================
-- 1) Função de acesso: todos os perfis, exceto visualizador
-- ============================================================
...
-- ============================================================
-- 2) Linhas dinâmicas: menu.bi / rota.bi = true para rh, dp1, dp2, financeiro
-- ============================================================
...
WHERE recurso IN ('menu', 'rota')
  AND acao = 'bi'
  AND perfil IN ('rh', 'dp1', 'dp2', 'financeiro');

-- ============================================================
-- 3) reset_permissoes_perfil recriada (base 104) com menu.bi/rota.bi = true
--    também para rh, dp1, dp2 e financeiro
-- ============================================================
...
BEGIN
  ...
  IF p_perfil = 'financeiro' THEN
    INSERT INTO public.permissoes_perfil (perfil, recurso, acao, permitido) VALUES
      ...
      ('financeiro', 'menu', 'bi', true),
      ('financeiro', 'rota', 'bi', true);
    RETURN;
  END IF;
