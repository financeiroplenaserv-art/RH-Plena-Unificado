-- Migração 104: reset_permissoes_perfil passa a incluir menu.bi / rota.bi
--
-- A função (migration 054) tem listas fixas por perfil e não conhecia as
-- permissões do módulo PerformanceLab (semente da migration 102): ao clicar
-- em "Restaurar padrão" na tela Permissões, as linhas menu.bi/rota.bi do
-- perfil eram apagadas e a aba sumia do menu até nova intervenção manual.
-- Recria a função idêntica à 054, acrescentando ao fim de cada bloco:
--   gestor / mesa / inspetoria  -> menu.bi e rota.bi = true
--   visualizador / rh / dp1 / dp2 / financeiro -> menu.bi e rota.bi = false
-- (mesmo escopo decidido pela gestão em 19/08/2026, migration 102).
-- Limitação conhecida documentada na 098 permanece para os demais recursos.
