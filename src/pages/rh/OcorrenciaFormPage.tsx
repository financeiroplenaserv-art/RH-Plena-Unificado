...
const TIPOS_COM_DOCUMENTO_OBRIGATORIO = TIPOS_OCORRENCIA.filter((t) => t.exigeAnexo).map(
  (t) => t.tipo
)

function exigeDocumento(tipo: string): boolean {
          ...
          <CardContent>
            {colabSelecionado ? (
              <div className="p-3 bg-blue-50 rounded-lg flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-slate-800">{colabSelecionado.nome_completo}</p>
                  <p className="text-xs text-slate-500">
                    Matrícula: {colabSelecionado.matricula} | {colabSelecionado.cargo}
        ...
        <Card className="border-slate-100 shadow-sm">
          ...
          <CardContent>
            <Select
              ...
              onValueChange={handleMacroGrupoChange}
              required
            >
              <SelectTrigger>
            ...
            {empresaSelecionada && (
              <div className="flex items-center gap-2 p-2 bg-blue-50 rounded-lg">
                ...
                <span className="text-xs text-blue-700">
                  Modelo pré-preenchido: {empresaSelecionada.nome} — CNPJ:{' '}
                  {empresaSelecionada.cnpj || 'Não informado'}
                </span>
              </div>
            )}
            <p className="text-xs text-slate-500">
              O texto abaixo foi pré-preenchido com base no tipo selecionado. Complete os campos
              [___] conforme necessidade.
            </p>
            <Textarea
              name="descricao"
