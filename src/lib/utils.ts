...
export function mascaraCNPJ(valor: string | null | undefined): string {
  if (!valor) return ''
...
export function validarCNPJ(cnpj: string | number | null | undefined): boolean {
  ...
  const calcDigito = (base: string, pesos: number[]): number => {
    const soma = base.split('').reduce((acc, digit, i) => acc + parseInt(digit, 10) * pesos[i], 0)
    const resto = soma % 11
