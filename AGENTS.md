...
supabase/
├── migrations/             # 111 migrations SQL (numeradas 001 a 111)
...
- Existem **111 migrations** em `supabase/migrations/` (numeradas `001_*` a `111_*`).
...
- **GRANT explícito obrigatório em tabela nova (mudança do Supabase, 30/10/2026)**: a partir dessa data o Supabase deixa de conceder acesso automático da Data API a tabelas novas no schema `public` — migration que cria tabela sem GRANT deixa a tabela inalcançável via supabase-js/PostgREST ("permission denied for table ...", erro que indica o GRANT a rodar). As tabelas existentes mantêm os grants atuais (nada a fazer retroativamente). Toda migration que criar tabela deve incluir, no mesmo arquivo: `grant select, insert, update, delete on public.<tabela> to authenticated;` e `... to service_role;` (e `grant select ... to anon;` só se a tabela for pública de propósito — em geral NÃO é; o app só usa `authenticated`). Os GRANTs não furam o RLS: as policies continuam valendo por cima. Nossas migrations atuais só têm `GRANT EXECUTE ON FUNCTION` — nenhuma concede tabela, porque valia o default antigo.
- Migrations recentes e críticas para segurança:
  ...
  - `109_ponto_espelho_periodo.sql` (decisão da gestão em 11/09/2026: `ponto_espelho_arquivos` ganha `periodo_inicio`/`periodo_fim` (cabeçalho do espelho) e `ponto_ate` (último dia com linha no PDF) — o cabeçalho traz o período completo, mas as linhas param na data de geração; o Calendário usava essa lacuna sem avisar, mostrando dias futuros como se fossem ponto. O app grava os metadados após o parse (`atualizarMetadadosPeriodo`, best-effort) e o Calendário mostra o banner "Ponto importado até dd/mm" + marca os dias sem registro como "previsto pela escala" (tracejado). UPDATE não tinha policy — criada com `is_editor()`, mesmo escopo do SELECT/INSERT da 094. Backup em `dados-locais/backup_ponto_espelho_arquivos_109_2026-09-11.json`. Aplicada via Management API em 11/09/2026 — atenção: `Get-Content` do PowerShell 5.1 precisa de `-Encoding UTF8` para SQL com acentos, senão o JSON do corpo quebra)
  - `110_exclusao_assistida_ocorrencias.sql` (decisão da gestão em 17/09/2026: exclusão de ocorrência deixa de ser SQL manual e vira fluxo assistido — ver a regra "Ocorrências — exclusão assistida" na seção 11. Tabela `ocorrencias_excluidas_log` (arquivo morto: cópia integral da ocorrência + filhas em JSONB, motivo, autor; retenção 90 dias com purge oportunista na própria RPC, sem pg_cron; SELECT só `is_admin()`, sem policy de escrita) + RPC `excluir_ocorrencia_admin` (SECURITY DEFINER: exige `is_admin()`, status 'Cancelada' e motivo; arquiva, exclui filhas e ocorrência numa transação e devolve os caminhos dos anexos para o frontend remover os arquivos do bucket — a RPC não alcança o storage). Aplicada via Management API em 17/09/2026)
  - `111_bi_todos_menos_visualizador.sql` (decisão da gestão em 24/09/2026: a aba PerformanceLab passa a ser acessível a **todos os perfis, exceto o visualizador** — antes (102) era só admin/adm/gestor/inspetoria/mesa. Três pontos alinhados: (1) `pode_ver_bi_performancelab()` ganha rh/dp1/dp2/financeiro (RLS das tabelas `bi_*`); (2) linhas dinâmicas `menu.bi`/`rota.bi` viradas para true em rh/dp1/dp2/financeiro (visualizador segue false); (3) `reset_permissoes_perfil` (104) recriada com os mesmos valores, para o "Restaurar padrão" não revogar o acesso. O botão "Atualizar agora" da página segue **só admin** (decisão de 05/09/2026, guarda da Edge Function). Sem mudança de frontend — menu/rota leem o banco. Aplicada via Management API em 24/09/2026)
...
- Local: `supabase/functions/sync-econtador/index.ts`.
- Job de máquina que porta a importação manual do e-Contador para o backend:
  busca Plena EA/Plena Tech, resolve departamentos por fuzzy match, grava
  colaboradores em lotes e registra o resultado no histórico compartilhado.
- Acesso somente com `Authorization: Bearer <ECONTADOR_CRON_KEY>`; não usar
  token de usuário nem expor a chave no frontend. O gateway fica com
  `verify_jwt: false`, como no `sync-performancelab`.
- Secrets: `ECONTADOR_CRON_KEY` (dedicada ao cron) e `ENCRYPTION_KEY` (mesma
  chave AES-256 usada pela function `econtador`).
...
- **Acesso**: dois caminhos na guarda da função — (1) job de máquina com `Authorization: Bearer <SYNC_CRON_KEY>` (chave aleatória dedicada — o projeto usa as novas API keys `sb_secret_`/`sb_publishable`, então a guarda não usa a service role; verificação de JWT do gateway desativada só nesta function); (2) **botão "Atualizar agora" da página PerformanceLab** (decisão da gestão, 05/09/2026: só admin) — JWT do usuário logado com perfil `admin`/`adm`, validado com `auth.getUser()` + leitura do `perfis.nivel_acesso` (a chave do cron nunca vai para o frontend: ficaria pública no bundle). Cliente em `src/services/performancelabApi.ts` (fetch + token da sessão, mesmo padrão do `econtadorApi`); ao concluir, a página recarrega os dados e o selo "Sincronizado em..." reflete a execução manual no `bi_sync_log`. Agendada **1x ao dia via pg_cron (decisão da gestão, 10/09/2026)**: `sync-performancelab-diario` 05h00 BRT (`0 8 * * *` UTC) — como o botão "Atualizar agora" passou a funcionar (CORS corrigido em 08/09), as execuções manuais cobrem o resto do dia. Até 10/09 eram 5 execuções diárias (03h00, 06h00, 07h30, 09h30 e 15h00 BRT), criadas porque **a API `pwbi` do PerformanceLab não é tempo real**: o dia anterior só aparece na API em algum momento da manhã, em horário variável (em 04/09/2026, às 03h00 BRT o dia 03/09 ainda não existia na API; às 09h05 já havia 15 visitas) — com 1 execução às 05h00, se o PL ainda não tiver publicado o dia anterior, usar o botão "Atualizar agora" mais tarde. Passo a passo em `docs/APLICAR_MIGRATION_102.md`. O sync é idempotente (upsert + reconciliação) — rodar várias vezes é seguro e barato.
...
- **CORS obrigatório — a function também é chamada pelo navegador (corrigido em 08/09/2026, v23)**: o botão "Atualizar agora" nunca tinha funcionado no browser — a function não respondia o preflight OPTIONS nem mandava `Access-Control-Allow-Origin`, então o navegador bloqueava a chamada ANTES do sync (sem registro no `bi_sync_log`, toast genérico). Agora segue o mesmo padrão das functions `econtador`/`suporte` (OPTIONS + ACAO refletindo a origem; lista opcional `ALLOWED_ORIGINS`). **Não remover o bloco CORS ao editar a function.**
- **RESOLVIDO (10/09/2026) — API do PerformanceLab rejeitando o login (HTTP 401)**: de ~09h30 BRT de 08/09 a 10/09 o PL respondeu `401 {"status":false,"error":"Unauthorized"}` no primeiro endpoint (`local`) — cron e botão falhavam igual. Causa: a senha da conta `plena.powerbi` foi trocada no PerformanceLab. Correção: secret `PLAB_SENHA` atualizado via `scripts/atualizar-senha-plab.ps1` (pede a senha na tela, grava pela Management API e dispara sync de teste com a chave do cron — usar o mesmo roteiro se a senha for trocada de novo; manter o script em ASCII puro, senão o PowerShell 5.1 quebra o parser). Sync de teste em 10/09 ok (76 locais, 570 checklists, 667 coletas, 238 eventos, 289 análises); o banner amarelo da página some sozinho no próximo sync bem-sucedido. Detalhe de diagnóstico: erros do PL gravam um trecho do corpo da resposta em `bi_sync_log.erro` (v22+).
- **Melhoria do toast do botão (deployada em 11/09/2026)**: o toast de erro do botão em `src/pages/bi/BiPerformanceLabPage.tsx` (`sincronizarAgora`) exibe o detalhe da causa (`err.message`) além da mensagem genérica.
...
- **Datas de colunas `date` — parsear SEMPRE como data local (bug de fuso corrigido em 05/09/2026)**: `new Date('2026-09-01')` interpreta a string como meia-noite **UTC** e, em fusos negativos (Brasil, UTC-3), exibe/computa o **dia anterior** — as 172 entregas de EPI de 01/09 apareciam como 31/08 para usuárias no Brasil e corretas para quem estava em fuso ≥ UTC. Use `parseDataLocal()` / `formatarData()` de `src/lib/utils.ts` (parse local) para qualquer coluna `date` (`data_entrega`, `data_ocorrencia`, `data_admissao`, `validade`, `data_vencimento` etc.). Testes em `src/lib/utils.test.ts`.
- **O CORH fala o horário de BRASÍLIA em tudo (decisão da gestão, 05/09/2026)**: o sistema só opera no Brasil, mas há operadora remota em fuso à frente — o relógio da máquina dela não pode vazar para os dados. Regras: (1) "hoje"/"agora" e períodos padrão (mês/semana corrente, datas padrão de formulários) usam `hojeBrasil()` / `agoraBrasil()` de `src/lib/utils.ts` — **nunca** `new Date().toISOString().split('T')[0]` (data em UTC) nem o "hoje" do dispositivo; (2) carimbos de hora (`created_at` etc.) são exibidos com `formatarDataHora()` / `formatarDataDeTimestamp()` (fixados em `America/Sao_Paulo`); (3) o que é GRAVADO como timestamp segue em UTC ISO (`new Date().toISOString()`) — correto, o banco guarda o instante; a conversão é só na exibição. `agoraBrasil()` devolve um Date cujos componentes LOCAIS refletem Brasília — usar só com getters locais. Corrigido em CEU, Ocorrências, RH, Extras, Escalas, Adicionais, Férias, VR, BI, Auditoria e Dashboard na mesma sessão do item acima.
- **Departamentos — filtro por nome_curto SEMPRE via `idsColaboradoresDoDepartamento` (corrigido em 05/09/2026, caso CBO/Aliança)**: o filtro por departamento não achava colaboradores por 3 causas sobrepostas: (1) ILIKE não casa texto legado sem acento com cadastro acentuado (`ALIANCA S A INDUSTRIA...` vs `Aliança S.A. Indústria...`); (2) **o cadastro tem linhas duplicadas do mesmo departamento** (uma com `nome_curto = 'CBO'`, outra sem nome_curto — o colaborador pode apontar para qualquer uma); (3) parte dos colaboradores só tem o texto, sem `departamento_id`. Solução: `idsColaboradoresDoDepartamento()` em `src/lib/departamentos.ts` resolve o departamento de cada colaborador com `encontrarDepartamentoFuzzy` **no cliente** e expande o alvo para linhas duplicadas (mesmo nome/nome_curto normalizado — `normalizarDepartamento` exportado para agrupamentos, ex.: `AutocompleteColaborador.buscarIdsDoGrupo`). **Nunca filtrar departamento com ILIKE no banco.** Distinção preservada: "CBO Macaé" NÃO entra no filtro "CBO" (postos diferentes). Aplicado em relatórios CEU, Movimentações, listagem de colaboradores e autocomplete. Validado contra produção: filtro "CBO" retorna 17 colaboradores (antes: 0). Testes em `src/lib/departamentos.test.ts`. Pendente de decisão da gestão: fundir as linhas duplicadas de `departamentos` (limpeza de dados — a fusão de locais do módulo Escalas, `backup_fusao_locais_2026-08-07.json`, é o precedente).
- **Departamentos — EXIBIÇÃO é sempre o nome_curto resolvido (decisão da gestão, 05/09/2026, commit `d72fbee`)**: nenhuma tela, PDF, Excel/TSV ou texto copiável mostra o texto legado de `colaboradores.departamento`. Use `nomeCurtoDepartamentoFuzzy()` — que ganhou o **fallback de linha irmã**: se a linha resolvida não tem nome_curto (duplicada legada), usa o da irmã com o mesmo nome normalizado (prefere Ativa; `DepartamentoFuzzy` ganhou `status`). A lista de resolução deve ser buscada SEM filtro de nome_curto (`select('id, nome, nome_curto, empresa_id, status')`) — **`useDepartamentos.listar()` filtra nome_curto NOT NULL e NÃO serve para resolução**. Migrados: CEU (recibos, nova entrega, lançamento rápido, exportações), RH (ficha, card da ocorrência, dialog da listagem), Dashboard, PDFs (incl. resolução de empresa, que usava ILIKE e nunca casava), autocomplete global (incl. selo "deste dept." por grupo, não por id exato), Falta Mobile (card + filtro por departamento), notificações de férias, balanço de extras. E **estancada a origem das duplicadas**: `useEContador.sincronizarDepartamentos` agora faz match fuzzy antes de criar departamento novo (o e-Contador manda o nome sem acento). `useExtras.verificarDuplicado` expande o grupo de duplicadas. Ficaram de fora de propósito: `testemunhas.departamento` (texto livre) e a prévia do e-Contador (texto cru da API externa antes do mapeamento).
...
- **Adicionais — contagem "vinculados/esperados" da tela Contratos é POR PERÍODO (decisão da gestão, 16/09/2026, caso Enseada)**: a coluna "# no período" conta colaboradores únicos cujo vínculo **se sobrepõe ao período selecionado** (`contarVinculosUnicosPorContrato(vinculos, inicio, fim)`) — antes somava o histórico de todos os períodos e um contrato de 2 vagas aparecia "3/2" por causa de vínculo encerrado. A tela ganhou o navegador "Período da contagem" (Anterior/Próximo, ciclo dia 20 a 19, persistido em `adicionais.contratos.ano/mes`, padrão = período que contém hoje via `periodoAdicionalDaData(agoraBrasil())`) e o modal de vinculados lista só o período escolhido. Alertas nos dois sentidos: "incompleto" (âmbar, faltam) e "excedente" (vermelho, sobram — não existia). Helpers `limitesPeriodoAdicional`/`periodoAdicionalDaData` em `src/lib/adicionais/calculoAdicionais.ts`, com testes.
...
- **Ocorrências**: visualização restrita; exclusão apenas `admin`/`adm` e somente via fluxo assistido (migration 110) — nunca SQL manual.
- **Ocorrências — listagem esconde Canceladas por padrão (decisão da gestão, 17/09/2026)**: o cancelamento é o destino formal do registro (soft delete), mas a cancelada visível poluía a lista e gerava pedido de exclusão física. O filtro Status da listagem nasce em **"Todos, exceto canceladas"** (sentinela `STATUS_FILTRO_EXCETO_CANCELADAS` em `src/lib/ocorrencias/filtroStatus.ts`, vira `neq('status','Cancelada')` no `useOcorrencias`); "Todos os status" e "Cancelada" continuam disponíveis. "Limpar" volta para a sentinela. Testes em `filtroStatus.test.ts`.
- **Ocorrências — exclusão assistida (decisão da gestão, 17/09/2026 — migration 110)**: a exclusão física existe só para lixo genuíno (teste, duplicata) e segue 3 travas: **só admin/adm**, **só status Cancelada** (cancelar primeiro pelo detalhe) e **motivo obrigatório**. O botão de lixeira da listagem só aparece nessas condições e chama `useOcorrencias.excluirDefinitivo` → RPC `excluir_ocorrencia_admin` (SECURITY DEFINER; as travas são conferidas de novo no banco), que numa transação arquiva a ocorrência e as filhas (anexos, testemunhas, aprovações, defesas) em `ocorrencias_excluidas_log` (retenção 90 dias, purge oportunista na própria RPC; SELECT só admin) e então exclui — o trigger de auditoria (061) registra o DELETE em `log_auditoria` sozinho. A RPC devolve os `caminhos_storage` e o frontend remove os arquivos do bucket `ocorrencia-anexos` em seguida (best-effort — arquivo órfão no bucket privado é inócuo). O botão antigo da listagem (lixeira em ocorrência NÃO cancelada que tentava DELETE direto, bloqueado por RLS para não-admin) foi removido: o caminho é sempre cancelar → excluir.
...
- **Ocorrências — lançar para colaborador inativo (decisão da gestão, 15/09/2026)**: a Nova Ocorrência permite lançar para inativo/demitido (caso esporádico, ex.: registro após o desligamento), mas **sem expor inativos por padrão** — o card Colaborador do formulário tem o seletor **Ativos | Inativos** (padrão Ativos; trocar remonta a busca restrita a inativos). `AutocompleteColaborador` ganhou a prop opcional `somenteInativos` (espelho da `somenteAtivos`); demais telas inalteradas. Implementação em `src/components/ocorrencias/ocorrencia-form/ColaboradorSection.tsx`.
...
- **BI — filtros persistem na sessão e a tabela de eventos ordena por Responsável/Local (15/09/2026)**: todos os filtros da página PerformanceLab (aba ativa, Pessoa/Local globais e os filtros de cada aba — busca, modelo/conclusão, tipo/motivo/inspetor/dia, status/SLA/assunto/responsável) usam `useFiltroPersistente` (`corh:filtros:bi.*`): navegar para outro módulo e voltar não perde mais os filtros (antes a página remontava e zerava tudo). O painel global ganhou `FiltrosAtivosBadge` (conta Pessoa/Local). O período de datas NÃO é persistido de propósito (o padrão "últimos 5 dias" é rolante). Os cabeçalhos **Local** e **Responsável** da tabela "Todos os eventos do período" são clicáveis e ciclam A→Z → Z→A → ordem por data (padrão da consulta); lógica pura `ordenarEventos` (ordenação estável, pt-BR, sem peso de acento; empates mantêm mais recente primeiro) em `src/lib/bi/agregacoes.ts`, com testes; a ordenação escolhida também persiste.
- **BI — tabela vazia oferece "Buscar desde o dia 1 do mês" (decisão da gestão, 22/09/2026)**: a janela padrão de 5 dias escondia registros um pouco mais antigos (caso Exclusive: inspeções de 16/09 invisíveis em 22/09, parecendo falha de integração). Quando a tabela de detalhe de qualquer aba (Checklists, Visitas, Eventos) vem vazia e o período aplicado começa depois do dia 1 do mês corrente, a célula vazia mostra o botão "Buscar desde 01/MM/AAAA" (`ofertaDiaUm`/`buscarDesdeDiaUm` em `BiPerformanceLabPage.tsx`), que aplica o período dia 1 → hoje e recarrega — a busca e os filtros da aba (persistidos) continuam valendo sobre o período maior. `CelulaVazia` ganhou a prop opcional `acao`. Na mesma sessão: o filtro global **Local** passou a listar o cadastro completo de `bi_locais` (união com os `site_nome` do período carregado, para cobrir nomes legados) — antes as opções vinham só dos registros carregados, e um contrato sem registros na janela (ex.: Exclusive nos últimos 5 dias) nem aparecia para ser escolhido antes de ampliar o período. `bi_locais` ganhou tipo (`BiLocal` em `src/types/bi.ts` + entrada em `database.ts`).
- **BI — detalhe do evento mostra "O que foi feito" (decisão da gestão, 24/09/2026)**: o card expandido da aba Eventos exibe, além de "Descrição do evento" (observacao), "Finalização" (`acoes_realizadas_finalizacao`) e "Histórico de análises", o bloco **"O que foi feito"** com `bi_eventos.acoes_realizadas` — campo que o sync já gravava mas a tela não mostrava (85 registros com texto humano, ex.: evento 886 com finalização em branco e ação preenchida). O helper `oQueFoiFeito` (`src/lib/bi/agregacoes.ts`) **descarta a nota automática do PerformanceLab** ("O PerformanceLab informa que este evento teve seu status alterado para crítico..." — ruído gravado pelo PL em ~metade dos registros quando o SLA estoura; prefixo `O PerformanceLab informa`). Testes em `agregacoes.test.ts`.
