...
- **Melhoria do toast do botão (commitada em 10/09/2026, ainda NÃO deployada)**: o toast de erro do botão em `src/pages/bi/BiPerformanceLabPage.tsx` (`sincronizarAgora`) passou a exibir o detalhe da causa (`err.message`) além da mensagem genérica. Está commitada, mas sem build/deploy (poupando crédito do Netlify) — entra no próximo deploy agrupado.
