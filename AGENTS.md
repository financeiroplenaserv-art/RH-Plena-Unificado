...
- **Datas de colunas `date` — parsear SEMPRE como data local (bug de fuso corrigido em 05/09/2026)**: `new Date('2026-09-01')` interpreta a string como meia-noite **UTC** e, em fusos negativos (Brasil, UTC-3), exibe/computa o **dia anterior** — as 172 entregas de EPI de 01/09 apareciam como 31/08 para usuárias no Brasil e corretas para quem estava em fuso ≥ UTC. Use `parseDataLocal()` / `formatarData()` de `src/lib/utils.ts` (parse local) para qualquer coluna `date` (`data_entrega`, `data_ocorrencia`, `data_admissao`, `validade`, `data_vencimento` etc.). Para "hoje" como `YYYY-MM-DD`, use `formatarDataInput(new Date())` — **nunca** `new Date().toISOString().split('T')[0]` (devolve a data em UTC). Timestamps (`created_at`, `updated_at`) seguem com `new Date()` normal — a conversão para local é o comportamento correto. Corrigido em CEU (recibos, relatórios, formulário, itens), Ocorrências (detalhe), RH (ficha do colaborador, alertas), Dashboard e nos parsers de importação do Flit (`importarFlit.ts` — a data importada **andava 1 dia para trás** no Brasil, não era só exibição). Testes em `src/lib/utils.test.ts`.
